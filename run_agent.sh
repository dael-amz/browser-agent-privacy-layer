#!/usr/bin/env bash
# One-command agent run: proxy + key preflight + holo task + enforced egress monitor + cleanup.
#
#   ./run_agent.sh                          # default terminal task
#   ./run_agent.sh "your own task prompt"   # custom task for the agent
#   PLVA_AUDIT=1 ./run_agent.sh "task"       # keep the memory-only viewer alive afterward
#   Press Esc twice during the run to abort it.
#
# Prereq: .env containing API_KEY=<Overshoot key>, or HAI_API_KEY=<H Company key>
#
...
DEFAULT_TASK="Open the Terminal application and run the command: echo plva-agent-ok"
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
...
case "$PROVIDER" in
  overshoot)
    ...
    if [[ -z "${OVERSHOOT_API_KEY:-}${API_KEY:-}" ]] \
      ...
      echo "ERROR: put API_KEY=<Overshoot key> in .env or ~/.holo/.env" >&2
      exit 1
    fi
    ;;
  hcompany)
    ...
    if [[ -z "${HAI_API_KEY:-}" ]] \
      ...
      echo "ERROR: put HAI_API_KEY=<H Company key> in .env, or run holo login (saves to ~/.holo/.env)" >&2
      exit 1
    fi
    ;;
  *)
...
#    PLVA_HOOK=test enables the proxy's test hooks for the hook-mode verify.
...
PROXY_LOG=$(mktemp /tmp/plva-proxy-agent.XXXXXX)
chmod 600 "$PROXY_LOG"
    ...
    if [[ "$VISUAL_DETECTOR" == 0 ]]; then
      ...
      if [[ ! -f "$VISUAL_MODEL" ]]; then
        ...
        if [[ "$DETECTOR_VERSION" == "v3" && -z "${PLVA_VISUAL_MODEL:-}" ]]; then
          echo "       Extract the frozen plva-v3 bundle to plva-v3/ or select v2." >&2
        fi
        exit 1
      fi
...
EGRESS_READY_FILE=$(mktemp /tmp/plva-agent-egress-ready.XXXXXX)
if [[ -n "${PLVA_EGRESS_STATUS_FILE:-}" ]]; then
  EGRESS_STATUS_FILE="$PLVA_EGRESS_STATUS_FILE"
  EGRESS_STATUS_EPHEMERAL=0
else
  EGRESS_STATUS_FILE=$(mktemp /tmp/plva-agent-egress-status.XXXXXX)
...
# 3) The agent task. Frame-bearing run artifacts go to a private ephemeral directory.
RUNS_DIR=$(mktemp -d /tmp/holo-agent-runs.XXXXXX)
chmod 700 "$RUNS_DIR"
...
if [[ -n "$ABORTED" ]]; then
  ...
  echo "--- holo exit: $HOLO_EXIT (0 = agent task completed end-to-end)"
fi
if [[ "$AUDIT_MODE" == 1 ]]; then
