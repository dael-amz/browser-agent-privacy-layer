...
PLVA is a fail-closed privacy proxy for the HoloDesktop computer-use agent. This guide covers
every run mode and switch. Install the environment once with `uv sync --group dev`. No API keys,
screenshots, transcripts, or vault contents belong in this repository; keys live in an untracked
`.env` only.

## Consumer demo

Launch the consumer app with:
...
browser storage. Spoken prompts, voice-read, and the mediator UI remain extension points.
...
PLVA_REDACT_LIFECYCLE=eager \
PLVA_AUDIT=1 \
./run_agent.sh "Open Terminal, run: echo plva-audit-ok, then report the output"
```
...
With the Vision engine, `run_agent.sh` enables the privacy core by default. The same OCR
...
For synthetic diagnosis, individual privacy features can be disabled without changing the
...
`PLVA_REDACT=1 ./run_agent.sh` enables redaction; `PLVA_REDACT=0` (the default) explicitly disables
...
```bash
uv run plva-benchmark \
  --live --allow-provider-spend --mode on --runner ./run_agent.sh \
  --output /tmp/cua-live-plva-on.json
```
...
Recheck the model-to-local-tool contract with synthetic data only:

```bash
uv run plva-tool-probe \
  --provider hcompany \
...
action is not parseable, so the free-text marker channel is the supported fallback.
