"""Local tool channel spike: registry, detection, teaching, and loop.

Holo3 exposes no native ``tools`` support (confirmed by the recorded upstream
contract findings), so a "tool call" here is a convention the proxy teaches and
parses: either a reserved ``plva_tool`` entry inside the model's existing
structured ``tool_calls`` action envelope, or an explicit ``⟦TOOL⟧…⟦/TOOL⟧``
marker inside free text. Execution is local and deterministic; results are fed
back through a bounded proxy inner loop. Logs carry tool names, channels, and
...
def find_tool_call(completion: dict[str, Any]) -> ToolCall | None:
    ...
    executor (fail closed); ordinary actions and answers return ``None``
    untouched.
    """
...
class ToolLoop:
    ...
    The token tool channel must gate this memory through its token-only
    contract before any tool can touch a real value.
    """
