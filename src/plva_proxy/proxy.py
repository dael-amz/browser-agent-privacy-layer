...
The proxy is the runtime's only model endpoint: loopback-bound, injecting the
provider credential and relaying bodies verbatim (unknown keys included, per
the recorded upstream contract findings). On top of that pass-through role it
provides the interception seam: optional hooks may mutate the outbound
...
forwarded; any hook or parse failure forwards nothing at all (fail closed).
Logs carry only privacy-safe metadata (byte counts, statuses, durations,
exception class names), never bodies, frames, or key material. Redaction and
placeholder resolution plug into these hooks.
"""
...
def _tag_request(
    ...
    """Test hook: observably tag the upstream request."""

    return document, {**headers, "x-plva-hook": "request"}


def _noop_rewrite_actions(document: dict[str, Any]) -> dict[str, Any]:
    """Test hook: decode and re-encode each action payload unchanged.

    Exercises the exact parse → mutate → re-serialize path that placeholder
    resolution uses. Unparseable action content fails closed.
    """
...
def _bananafy_call(call: dict[str, Any]) -> list[str]:
    ...
    recorded upstream capture showed) and args nested under an ``args`` object.
    Returns the keys changed, for a privacy-safe log: key names only, never
    values.
    """
...
def image_replacement_hook(image_path: Path) -> RequestHook:
    ...
    (fail closed; a rehearsal for real screenshot redaction).
    """
...
def grammar_capture_hook(out_path: Path) -> RequestHook:
    ...
    The local tool channel needs the runtime's ``structured_outputs`` value to
    judge whether a novel tool action is even grammatically admissible. The
    snapshot records request keys, model id, ``structured_outputs``, and
    ``chat_template_kwargs`` and deliberately nothing else: the message
    history, the only part that can carry frames or values, is never written.
    """
...
class FrameStore:
    ...
    with the process. Holds only post-redaction pixels, exactly what the
    model sees.
    """
...
class CallStore:
    ...
    persisted anywhere; dropped with the process.
    """
...
def frame_redaction_hook(
    ...
    *,
    ...
    """Build a request hook that redacts every outbound screenshot.
    ...
    never mutated). Requests without a screenshot pass through untouched;
    any redaction failure raises so no raw frame can ever leave.
    """
...
def _assemble_sse_completion(raw: bytes) -> dict[str, Any]:
    ...
    or exotic stream raises so it is never re-emitted to the executor.
    """
...
def add_viewer_routes(
    ...
    *,
    ...
    async def run_viewer_tool(request: Request) -> Response:
        """Mandatory fallback: proxy/app-initiated op, no model marker needed."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="none",
        help="traffic mutation hooks: none = pass-through, test = tag/no-op test hooks, "
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="enable the session vault, placeholder chips, resolution, and history scrub",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--privacy-tools",
        action="store_true",
        help="enable the token tool channel (⟦PLVA_TOOL:…⟧ sort/select via the local LLM)",
    )
    parser.add_argument(
        "--privacy-mediator",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="enable the local tool channel: teaching, detection, bounded tool loop",
    )
    parser.add_argument(
        "--tools-max-rounds",
        type=int,
