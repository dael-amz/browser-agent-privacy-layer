"""Placeholder-preserving semantic operations via the local LLM.
...
token, and the completion is then discarded. The completion may carry
cleartext (that is allowed for a zero-egress local executor and it never
leaves this process) while the result folded into the CUA's next observation
is tokens-only by construction: mapped, membership-validated, never free text.

Invocation arrives via the local tool marker channel
