...
class StubRedactor:
    """Deterministic screenshot-detector stub with configurable fixture spans or no detections."""
...
class ToolChannelSupport(Protocol):
    """What the privacy hooks need from a token tool channel with mediator automation."""
...
def privacy_request_hook(
    ...
    *,
    ...
    def apply(
        ...
        scrubbed = (
            scrubber.scrub(texts, semantic_mask=semantic_mask) if texts and history_scrub else texts
        )
...
def _replace_variants(text: str, variants: list[tuple[str, tuple[str, str]]]) -> str:
    scrubbed = text
