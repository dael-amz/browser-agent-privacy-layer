...
## PLVA v3 visual detector

This bundle contains the safety-selected v10 detector exported through
Ultralytics 8.4.92. Its source checkpoint is recorded as AGPL-3.0-only. The
bundle is development-only and not release eligible; closed-source deployment
requires an Ultralytics Enterprise license and review of all training-data and
base-checkpoint obligations. Exact hashes and blockers are in `snapshot.json`
and the detector manifest under `runtime/training/artifacts/`.
