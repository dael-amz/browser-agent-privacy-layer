"""Step 13 tool channel + Step 7 mediator automation over the proxy seams.

Implements the Step 6.5 recommendation: the CUA invokes a local operation by
...
Routing: ``sort`` with no instruction is the deterministic library (A) —
resolve locally, order lexicographically, return tokens; ``sort`` with an
instruction and ``select`` delegate to the sandboxed local model (B) through
:class:`SemanticExecutor`, whose return is tokens-only by construction.

Step 7 automation: on an approval-gated resolution denial the channel consults
...
class ToolChannel:
    ...
    def _deterministic_sort(self, tokens: tuple[str, ...], request_id: str | None) -> str:
        """Library (A): resolve locally, order lexicographically, emit tokens only."""
