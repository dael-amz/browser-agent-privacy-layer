...
class HistoryScrubber:
    ...
    def scrub(self, texts: tuple[str, ...]) -> tuple[str, ...]:
        ...
        try:
            ...
            self._set_diagnostics("failed", len(texts), plain_hits, 0, 0)
            raise PrivacyError("history classifier failed") from exc
        if len(classifications) != len(plain):
            self._set_diagnostics("failed", len(texts), plain_hits, 0, cache_hits)
        ...
        self._set_diagnostics("passed", len(texts), plain_hits, semantic_hits, cache_hits)
        return scrubbed
    ...
    def _apply_classification(
        ...
        for raw in values:
            ...
            if self._vault.safety_level(label) == "blocked":
                # Blocked values must be removed from provider-bound history but
                # must never enter the resolvable vault. Use an opaque shape that
                # cannot match the session placeholder grammar.
                replacement = f"[PLVA_BLOCKED_{_normalize_class(label)}]"
            else:
                replacement = self._vault.store(label, text[start:end])
